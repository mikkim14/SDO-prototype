...
$category_config = [
    ...
    'water' => ['name' => 'Water', 'icon' => 'fa-droplet', 'color' => 'blue', 'table' => 'tblwater', 'year_col' => null],
    'treated_water' => ['name' => 'Treated Water', 'icon' => 'fa-faucet', 'color' => 'cyan', 'table' => 'tbltreatedwater', 'year_col' => null],
...
if ($config['year_col']) {
    ...
    if (strpos($year_col, '(') !== false) {
        $query = "SELECT DISTINCT $year_col as year FROM {$config['table']} ORDER BY year DESC";
    } else {
        $query = "SELECT DISTINCT $year_col as year FROM {$config['table']} WHERE $year_col IS NOT NULL ORDER BY year DESC";
    }
...
<div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm table-auto" id="category-table">
                <thead class="bg-gray-50">
                    <tr>
                        <?php foreach (array_keys($detailed_records[0]) as $col): ?>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap"><?php echo htmlspecialchars($col); ?></th>
                        <?php endforeach; ?>
                    </tr>
                    <tr class="bg-white">
                        <?php $colIdx = 0; foreach (array_keys($detailed_records[0]) as $col): ?>
                        <th class="px-2 py-1"><input type="text" class="w-full min-w-[80px] px-2 py-1 text-xs border rounded filter-input" data-table="category-table" data-col="<?php echo $colIdx++; ?>" placeholder="Search..."></th>
                        <?php endforeach; ?>
                    </tr>
                </thead>
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-2 text-xs whitespace-nowrap max-w-xs truncate" title="<?php echo htmlspecialchars($val ?? ''); ?>"><?php echo htmlspecialchars($val ?? ''); ?></td>
                        <?php endforeach; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
