<?php
$page_title = 'Waste Unsegregated Report';
...
// Get campuses
$campuses = [];
$stmt = $db->prepare("SELECT DISTINCT Campus FROM tblsolidwasteunsegregated ORDER BY Campus");
$stmt->execute();
...
// Get years
$years = [];
$stmt = $db->prepare("SELECT DISTINCT Year FROM tblsolidwasteunsegregated WHERE Year IS NOT NULL ORDER BY Year DESC");
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
    $years[] = $row['Year'];
...
// Fetch records
$waste_unseg_records = [];
$query = "SELECT * FROM tblsolidwasteunsegregated $where ORDER BY Year DESC, Month, Quarter LIMIT 1000";
...
foreach ($waste_unseg_records as $record) {
    ...
    $total_landfill_kg += $record['SentToLandfillKG'] ?? 0;
    $total_landfill_t += $record['SentToLandfillTONS'] ?? 0;
...
?>
                    ...
                    <div class="max-w-7xl">
                        ...
                        <div class="mb-6">
                            ...
                            <p class="text-gray-600 mt-1">Mixed Municipal Solid Waste Management</p>
                        </div>
                        ...
                        <!-- Statistics Cards -->
                        <?php if (!empty($waste_unseg_records)): ?>
                            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                                    <div class="text-blue-600 text-sm font-semibold uppercase mb-2">Total Records</div>
                                    <div class="text-3xl font-bold text-blue-900"><?php echo count($waste_unseg_records); ?></div>
                                </div>
                                ...
                                <div class="bg-orange-50 border border-orange-200 rounded-lg p-6">
                                    ...
                                    <div class="text-3xl font-bold text-orange-900"><?php echo number_format($total_landfill_kg, 2); ?> kg</div>
                                </div>
                            ...
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
                                    <h2 class="text-xl font-semibold text-gray-800">
                                        <i class="fas fa-dumpster text-orange-600 mr-2"></i>
                                        Waste Unsegregated Records (<span id="waste-unseg-count"><?php echo count($waste_unseg_records); ?></span>)
                                    </h2>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full text-sm" id="waste-unseg-table">
                                        <thead class="bg-gray-50">
                                            <tr>
                                                <?php foreach (array_keys($waste_unseg_records[0]) as $col): ?>
                                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase"><?php echo htmlspecialchars($col); ?></th>
                                                <?php endforeach; ?>
                                            </tr>
                                            <tr class="bg-white">
                                                <?php $colIdx = 0; foreach (array_keys($waste_unseg_records[0]) as $col): ?>
                        ...
                        <?php else: ?>
                            <div class="bg-white rounded-lg shadow-md p-12 text-center">
                                ...
                                <p class="text-gray-600 text-lg">No waste unsegregated records found</p>
...
<script>
// Initialize waste unsegregated table
if (document.getElementById('waste-unseg-table')) {
    initializeTable('waste-unseg');
}
</script>
