...
// Fetch all accommodation records (apply office/campus visibility)
$records = AccessControl::fetchRecords($db, 'tblaccommodation', $filter, 'date DESC', 100);

// Handle form submission
if (Helper::isPostRequest()) {
    if (!$can_input) {
        $error_msg = 'Your office is read-only for Accommodation. Contact Central to modify data.';
    }
    ...
    if ($validator->fails()) {
        ...
        try {
            ...
            if ($record_id) {
                ...
                $query = "UPDATE tblaccommodation SET date = ?, guests = ?, nights = ? WHERE id = ? AND campus = ? AND office = ?";
                $stmt = $db->prepare($query);
                if ($stmt) {
                    $stmt->bind_param("siiiss", $date, $guests, $nights, $record_id, $user['campus'], $user['office']);
                    if ($stmt->execute()) {
                ...
                // Insert (include office from session)
                $query = "INSERT INTO tblaccommodation (campus, office, date, guests, nights) VALUES (?, ?, ?, ?, ?)";
                $stmt = $db->prepare($query);
                if ($stmt) {
...
if (Helper::isGetRequest() && isset($_GET['delete_id'])) {
    ...
    if (!$can_input) {
        $error_msg = 'You do not have permission to delete accommodation records.';
    } else {
        // Delete with campus + office enforcement
        $query = "DELETE FROM tblaccommodation WHERE id = ? AND campus = ? AND office = ?";
        $stmt = $db->prepare($query);
        if ($stmt) {
