<?php
$page_title = 'Category Report - RGO';
require_once 'tailwind-header.php';

$category = $_GET['cat'] ?? 'lpg';
$selected_campus = $user['campus'];
$selected_year = $_GET['year'] ?? null;

// Category configuration
$category_config = [
    'lpg' => ['name' => 'LPG', 'icon' => 'fa-fire', 'color' => 'red', 'table' => 'tbllpg', 'year_col' => 'YearTransact'],
    'food' => ['name' => 'Food Consumption', 'icon' => 'fa-utensils', 'color' => 'amber', 'table' => 'tblfoodwaste', 'year_col' => 'YearTransaction'],
];

$config = $category_config[$category] ?? $category_config['lpg'];

// Get years
$years = [];
$year_query = "SELECT DISTINCT {$config['year_col']} as year FROM {$config['table']} WHERE {$config['year_col']} IS NOT NULL ORDER BY year DESC";
...
function downloadCSV() {
    ...
    csv += '\n\n=== SUMMARY ===\n';
    csv += 'Category,Records,Consumption,Unit,CO2 (kg)\n';
    csv += '"<?php echo addslashes($data['name']); ?>",<?php echo $data['records']; ?>,<?php echo $data['consumption']; ?>,"<?php echo addslashes($data['unit']); ?>",<?php echo $data['kg_co2']; ?>\n';
