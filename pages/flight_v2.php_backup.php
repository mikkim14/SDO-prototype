...
$records = [];
$query = "SELECT * FROM tblflight WHERE campus = ? ORDER BY date DESC LIMIT 100";
...
if (Helper::isPostRequest()) {
    ...
    if ($validator->fails()) {
        ...
        try {
            if ($record_id) {
                // Update
                $query = "UPDATE tblflight SET date = ?, destination = ?, travelers = ?, fuel_consumed = ? WHERE id = ? AND campus = ?";
                $stmt = $db->prepare($query);
                if ($stmt) {
                    $stmt->bind_param("sddis", $date, $destination, $travelers, $fuel_consumed, $record_id, $user['campus']);
                    if ($stmt->execute()) {
                ...
                // Insert
                $query = "INSERT INTO tblflight (campus, date, destination, travelers, fuel_consumed) VALUES (?, ?, ?, ?, ?)";
                $stmt = $db->prepare($query);
                if ($stmt) {
            ...
            // Refresh records
            $stmt = $db->prepare("SELECT * FROM tblflight WHERE campus = ? ORDER BY date DESC LIMIT 100");
            $stmt->bind_param("s", $user['campus']);
            $stmt->execute();
            $result = $stmt->get_result();
            $records = $result->fetch_all(MYSQLI_ASSOC);
...
if (Helper::isGetRequest() && isset($_GET['delete_id'])) {
    ...
    $query = "DELETE FROM tblflight WHERE id = ? AND campus = ?";
    $stmt = $db->prepare($query);
    if ($stmt) {
