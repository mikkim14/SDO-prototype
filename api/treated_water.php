...
try {
    switch ($action) {
        ...
        case 'add_record':
            ...
            $volume = (float)Helper::getPost('volume', 0);
            $stmt = $db->prepare("INSERT INTO treated_water (campus, office, date, volume) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("sssd", $user['campus'], $user['office'], $date, $volume);
            if ($stmt->execute()) {
                Helper::logActivity($db, 'Added Treated Water Record for ' . $user['campus'] . ' - Office: ' . $user['office'], 'Water Report');
        ...
        case 'update_record':
            ...
            $date = Helper::getPost('date', '');
            $volume = (float)Helper::getPost('volume', 0);
            $stmt = $db->prepare("UPDATE treated_water SET date = ?, volume = ? WHERE id = ? AND campus = ? AND office = ?");    
            $stmt->bind_param("sdiss", $date, $volume, $id, $user['campus'], $user['office']);
            if ($stmt->execute()) {
        ...
        case 'delete_record':
            $id = (int)Helper::getPost('id', 0);
            $stmt = $db->prepare("DELETE FROM treated_water WHERE id = ? AND campus = ? AND office = ?");
