...
try {
    switch ($action) {
        case 'get_records':
            ...
            $stmt = $db->prepare("SELECT * FROM tbltreatedwater WHERE campus = ? ORDER BY date DESC LIMIT ? OFFSET ?");
        ...
        case 'add_record':
            ...
            $stmt = $db->prepare("INSERT INTO tbltreatedwater (campus, date, amount) VALUES (?, ?, ?)");
            $stmt->bind_param("ssd", $user['campus'], $date, $amount);
            
            if ($stmt->execute()) {
                Helper::logActivity($db, 'Added Treated Water Record for ' . $user['campus'], 'Treated Water Report');
        ...
        case 'update_record':
            ...
            $stmt = $db->prepare("UPDATE tbltreatedwater SET date = ?, amount = ? WHERE id = ? AND campus = ?");
            $stmt->bind_param("sdis", $date, $amount, $id, $user['campus']);
            
            if ($stmt->execute()) {
                Helper::logActivity($db, 'Updated Treated Water Record (ID: ' . $id . ')', 'Treated Water Report');
        ...
        case 'delete_record':
            ...
            $stmt = $db->prepare("DELETE FROM tbltreatedwater WHERE id = ? AND campus = ?");
            $stmt->bind_param("is", $id, $user['campus']);
            
            if ($stmt->execute()) {
                Helper::logActivity($db, 'Deleted Treated Water Record (ID: ' . $id . ')', 'Treated Water Report');
