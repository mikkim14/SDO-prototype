...
try {
    switch ($action) {
        case 'get_records':
            ...
            $sql = "SELECT * FROM solid_waste_segregated";
        ...
        case 'add_record':
            ...
            $waste_type = Helper::getPost('waste_type', '');
            $quantity = (float)Helper::getPost('quantity', 0);
            $stmt = $db->prepare("INSERT INTO solid_waste_segregated (campus, office, date, waste_type, quantity) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssd", $user['campus'], $user['office'], $date, $waste_type, $quantity);
            if ($stmt->execute()) {
                Helper::logActivity($db, 'Added Waste Segregation Record for ' . $user['campus'] . ' - Office: ' . $user['office'], 'Waste Report');
        ...
        case 'update_record':
            ...
            $waste_type = Helper::getPost('waste_type', '');
            $quantity = (float)Helper::getPost('quantity', 0);
            $stmt = $db->prepare("UPDATE solid_waste_segregated SET date = ?, waste_type = ?, quantity = ? WHERE id = ? AND campus = ? AND office = ?");    
            $stmt->bind_param("ssdsiss", $date, $waste_type, $quantity, $id, $user['campus'], $user['office']);
            $stmt->execute() ? Response::success([], 'Updated') : Response::error('Failed', 500);
            $stmt->close();
            break;
        case 'delete_record':
            $id = (int)Helper::getPost('id', 0);
            $stmt = $db->prepare("DELETE FROM solid_waste_segregated WHERE id = ? AND campus = ? AND office = ?");
